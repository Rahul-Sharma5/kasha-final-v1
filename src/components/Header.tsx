...
const Header: React.FC<HeaderProps> = ({ toggleTheme, theme }) => {
  ...
  const getNavLinkClass = (isActive: boolean) => {
      return `px-3 py-2 rounded-md text-sm font-medium transition-colors ${
          isActive 
          ? 'text-brand-gold font-bold drop-shadow-[0_0_8px_rgba(67,175,152,0.4)]' 
          : 'text-gray-500 dark:text-gray-300 hover:text-brand-gold'
      }`;
  };
  ...
  const getMobileNavLinkClass = (isActive: boolean) => {
      return `block px-3 py-2 rounded-md text-base font-medium transition-colors ${
          isActive 
          ? 'text-brand-gold bg-gray-100 dark:bg-gray-800 font-bold' 
          : 'text-gray-600 dark:text-gray-300 hover:text-brand-gold hover:bg-gray-200 dark:hover:bg-gray-700'
      }`;
  };
